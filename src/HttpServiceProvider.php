...
use Lamen\Http\Server\Facades\Server;
use Lamen\Http\Coroutine\MySqlConnection;
use Lamen\Http\Commands\HttpServerCommand;
use Swoole\Websocket\Server as WebsocketServer;
use Lamen\Http\Coroutine\Connectors\MySqlConnector;
use Lamen\Http\Task\Connectors\SwooleTaskConnector;
...
abstract class HttpServiceProvider extends ServiceProvider
{
    ...
    {
        $this->publishes([
            __DIR__ . '/../config/lamen.php' => base_path('config/lamen.php'),
            __DIR__ . '/../config/swoole_http.php' => base_path('config/swoole_http.php'),
            __DIR__ . '/../config/swoole_lumen.php' => base_path('config/swoole_lumen.php'),
            ...
            __DIR__ . '/FrameWork/artisan_lumen' => base_path('artisan_lumen'),
            __DIR__ . '/FrameWork/public/index.php' => base_path('public/lamen.php')
        ], 'lamen-swoole');
        ...
        if (!defined('FRAME_WORK_NAME')) {
            define('FRAME_WORK_NAME', $this->framework);
        }
    }
    ...
    {
        $this->mergeConfigFrom(__DIR__ . '/../config/swoole_lumen.php', 'swoole_lumen');
        ...
        if ($this->framework == 'lumen') {
            $config = $this->app['config']->get('swoole_lumen', []);
            $this->app['config']->set('swoole_http', $config);
        }
    }
